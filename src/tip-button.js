    ...
    if (this.state.isLoading) {
      return (
        <div className="post-button">
          <Loader style={{float: "left"}} color="#f11c08" size="20px"/>
        </div>
      );
    }
    else {
      if (this.state.hasTipped) {
        return (
          <div className="post-button">
            <Glyphicon glyph='heart' style={{color: "red"}}/> {this.props.post.tips}
          </div>
        );
      }
      else {
        return (
          <div className="post-button" onClick={this.tipUpdate.bind(null, this.props.post)}>
            <Glyphicon glyph='heart'/> {this.props.post.tips}
          </div>
        );
      }
    }
